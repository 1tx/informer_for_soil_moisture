...
# ------------------------------------------------------------------------------------------------------------------------------
 ...
 # ------------------------------------------------------------------------------------------------------------------------------
	#Load forcing data
        ...
        forcing_list = []
        day_list = []
        for year in range(begin_year, end_year+1): # 左开右闭区间
...
# ------------------------------------------------------------------------------------------------------------------------------
	...
	#Load static_norm data
        if not os.path.exists(PATH + 'static_norm.npy'):
          ...
          #  static = np.where(np.isnan(static) & (mask[:, :, np.newaxis]), 0, static)
          #  test2 = static[:,:,1]
	...
	#Partition and create datasets
        ...
        assert forcing.shape[0] == label.shape[0]&land_surface.shape[0] == label.shape[0], "X(t) /= label(t)"
        # get shape
        ...
        N = 365 * len(self.test_year)+cfg['seq_len']+cfg['forcast_time']+14  # 验证的年份，加上seq_len+forcast_time+14
	...
	#生成训练集，并且将forcing+land_surface加入
...
# normalize
		...
		#####region
                if cfg['normalize_type'] in ['region']:
                    # 最大，最小标准化，第一个维纬度最大最小，第二个维度latitude，第三个维度longitude，第四个维度特征
